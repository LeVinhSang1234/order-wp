...
//     $table_orders = $wpdb->prefix . 'orders'; // Bảng lưu đơn hàng
...
//     // Lưu danh sách cart_id dưới dạng mảng string
// $cart_ids = [];

// foreach ($products as $product) {
//     $cart_ids[] = (string) $product['id']; // Chuyển ID sang string

//     // Cập nhật trạng thái is_done trong bảng cart
//     $wpdb->update(
//         $table_cart,
//         ['is_done' => 1],  // Đánh dấu là đã yêu cầu
//         ['id' => $product['id'], 'user_id' => $user_id]
//     );
// }

// // Chuyển mảng cart_ids thành chuỗi JSON để lưu vào order
// $cart_ids_json = json_encode($cart_ids);

//     // Tạo đơn hàng mới và lưu cart_ids vào đó
//     $order_data = [
//         'user_id' => $user_id,
//         'cart_ids' => $cart_ids_json, // Lưu danh sách cart_id
...
function submit_all_cart_handler() {
    ...
    // Lưu danh sách cart_id và tính tổng giá trị đơn hàng
    $cart_ids = [];
    $total_price = 0;

    foreach ($products as $product) {
        $cart_ids[] = (string) $product['id'];
        $total_price += floatval($product['price']) * intval($product['quantity']); // Tính tổng giá trị đơn hàng
    ...
    // Tính phí dịch vụ theo tổng giá trị đơn hàng
    ...
    // Chuyển mảng cart_ids thành JSON để lưu vào order
    $cart_ids_json = json_encode($cart_ids);
